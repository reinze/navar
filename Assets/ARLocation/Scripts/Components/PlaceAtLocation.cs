...
namespace ARLocation
{
    ...
    public class PlaceAtLocation : MonoBehaviour
    {
        ...
        public class StateData
        {
            ...
            public bool Paused;
            // NEW: Flag to track if initial placement for hidden objects is done
            public bool IsInitialHiddenPlacementDone = false;
        }
        ...
        void Initialize()
        {
            ...
            if (!hasInitialized)
            {
                // Hiding object here if option is true
                if (PlacementOptions.HideObjectUntilItIsPlaced)
                {
                    Misc.HideGameObject(gameObject);
                    state.IsInitialHiddenPlacementDone = false; // Reset flag
                }
                else
                {
                    state.IsInitialHiddenPlacementDone = true; // No initial hidden state
                }
        ...
        private void ProviderRestarted()
        {
            ...
            state.IsInitialHiddenPlacementDone = false; // Reset flag on provider restart
        }
        ...
        public void UpdatePosition(Location deviceLocation, bool forceUpdate = false)
        {
            ...
            // NEW: Handle initial placement for hidden objects
            if (PlacementOptions.HideObjectUntilItIsPlaced && !state.IsInitialHiddenPlacementDone)
            {
                transform.position = targetPosition; // Set position directly, no smooth move
                state.IsInitialHiddenPlacementDone = true; // Mark as initial placement done
                PositionUpdated(); // Call PositionUpdated to potentially show object
            }
            else if (useSmoothMove && gameObject.activeInHierarchy && state.PositionUpdatedCount > 0) // Only use smooth move if active and not first pos update
            {
            ...
            else // Direct placement for non-smooth or first position update
            {
        ...
        private void PositionUpdated()
        {
            if (PlacementOptions.HideObjectUntilItIsPlaced)
            {
                // Only show object if it reaches or exceeds the threshold
        ...
        public static GameObject CreatePlacedInstance(GameObject go, Location location, PlaceAtOptions options, bool useDebugMode = false)
        {
            ...
            // Coba dapatkan TextMeshPro dan set teksnya jika ada
            var tmpComponent = instance.GetComponentInChildren<TextMeshPro>();
            if (tmpComponent != null)
            {
                tmpComponent.text = location.Label;
            }
        ...
        {
            // Perhatikan: GameObject diatur ke inactive di sini, yang memicu masalah jika coroutine mencoba berjalan
            // Namun, logika di UpdatePosition() sekarang menangani ini dengan lebih baik.
            go.SetActive(false);
            ...
            // Penting: Aktifkan GameObject HANYA SETELAH PlaceAtLocation ditambahkan dan dikonfigurasi.
            // Ini akan memicu metode Start() dan Initialize() dari PlaceAtLocation.
            go.SetActive(true);
