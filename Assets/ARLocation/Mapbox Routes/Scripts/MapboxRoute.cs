...
namespace ARLocation.MapboxRoutes
{
    public class MapboxRoute : MonoBehaviour
    {
        ...
        void Update()
        {
            if (NumberOfSteps > 0)
            {
                ...
                for (int i = 0; i < NumberOfSteps; i++)
                {
                    ...
                    {
                        if (i == s.CurrentTargetIndex)
                        {
                            if (isStart)
                            {
                                if (closestDist == 0 || closestDist > signpostEventArgs.Distance)
                                {
                                    closestDist = signpostEventArgs.Distance;
                                }
                                else if (signpostEventArgs.Distance - closestDist > 10)
                                {
                                    //menuController.EndRoute();
                        ...
                        {
                            //print(signpostEventArgs.Distance);
        ...
        // Tambahkan ke dalam class MapboxRoute.cs (bagian public method)
        public Vector3 GetStepPosition(int index)
        {
            if (index >= 0 && index < s.StepsPlaceAtInstances.Count)
            {
                return s.StepsPlaceAtInstances[index].transform.position;
            }
        ...
        public void SetTarget(int index)
        {
            ...
            if (isValidTargetIndex(index))
            {
                ...
                }print(s.CurrentTargetIndex+" "+ (NumberOfSteps - 1));
            }
            else
            {
                Debug.LogWarning($"SetTarget: Invalid target index {index}. Number of steps: {NumberOfSteps}");
                // NEW: Jika index invalid, dan itu adalah setelah langkah terakhir, kemungkinan sudah mencapai akhir rute
                if (index >= NumberOfSteps && NumberOfSteps > 0 && s.CurrentTargetIndex == (NumberOfSteps - 1))
                {
                    print("sampai");
                    menuController.EndTrip();
                }
            }
        }
